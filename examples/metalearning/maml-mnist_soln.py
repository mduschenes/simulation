...
def random_init(std, nhid):
    ### YOUR CODE HERE  
    return {'W1': np.random.normal(0, std, size=(28*28,nhid)),
            'b1': np.random.normal(0., std, size=nhid),
            'w2': np.random.normal(0., std, size=(nhid,2)),
            'b2': np.random.normal(0., std,size=2)}
    ### END CODE 
...
def gd_step(cost, params, lrate):
    ...
    ### YOUR CODE HERE
    grad_fn = ag.grad(cost)
    grad = grad_fn(params)
    return {k: params[k] - lrate * grad[k] for k in params}   
    ### END CODE
...
class InnerObjective:
    ...
    def __call__(self, params):
        ### YOUR CODE HERE 
        return  -np.mean(np.sum(self.y * np.log(1e-12 + net_predict(params,self.x)), axis=1))
        ### END CODE
...
class MetaObjective:
    ...
    def __call__(self, params, return_traj=False):
        ...
        for i in range(self.num_steps):
            params = gd_step(inner_obj, params, self.inner_lrate)
            trajectory.append(params)
        final_cost = inner_obj(params)
        ### END STUDENT CODE

        if self.acc==True:  
            ### BEGIN STUDENT CODE 
            pred = net_predict(params,self.x).argmax(1)
            target = self.y.argmax(1)
            val_acc = 100.*float((pred == target).sum())/float(target.size) 
            print("Accuracy after finetuning = ", val_acc) 
            ### END STUDENT CODE
...
def train():
    ...
    for i in range(OUTER_STEPS):
        ### YOUR CODE HERE
        x, y,_,_ = data_gen.sample_dataset(NDATA)
