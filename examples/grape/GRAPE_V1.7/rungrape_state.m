function GRinfo = rungrape_state(InputName,div,askplot,FileOpt)
%% %%%%%%%%%%%%%%%%%%%%%%% INFORMATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin<1
   disp(' ')
   disp('                                 GRAPE 1.3                   ')
...
%% %%%%%%%%%%%%%%%%%%%%%%%% DEFAULT INPUTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
if (nargin < 2 | isempty(div)); div = 10    ; end;
...
path(path,[pwd filesep 'controls'])
...
gra.RHO_init = gra.Ud*gra.RHO_in*gra.Ud';                  %Unitary for which GRAPE will be optimized
...
[Fid P X] = calculate_robustfidelity_state(u);
gra.F(1)=Fid;
counter=0;
n=0;
...
while (Fid <targ_fide)
    counter=counter+1;
    [epsilon u]=maximise_robustfidelity_state(P,X,u,epsilon,counter);
    ...
    if(gra.F(counter)>saving_fidel)
        if(mod(n,iter_no)==0)
            gra.u=u;
            [gra.IDEALfidelity X1] = calculate_fidelity_state(u);
            gra.Usim=X1(:,:,gra.N+1);
            gra.RFfidelity = gra.F(length(gra.F));
            GRinfo = saveGRstr_state(gra);
...
gra.reason = {'Fidelity Not Improving','Target Fidelity Reached','No Iterations Done'};
