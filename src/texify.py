...
def findstring(string,strings,types,prefixes,labels,replacements,default=None,regex=False,usetex=True):
	...
	latex = None
	func = lambda string,latex,iloc,labels:latex
	iloc = None
	...
	for func,iloc in zip(funcs,ilocs):
		latex = func(string,latex,iloc,labels)
...
class Texify(object):
	...
	def __init__(self,texstrings={},texargs={},texlabels={},usetex=True):
		...
		args_default = {
			'bases':{'monomial':1,'polynomial':0,'taylorseries':1,},
			'order':2,
			'basis':3,
		...
		args_keyed = ['iloc']
		args_dependent = {'groups': lambda args:{x: args['inputs'].get(x) for x in args['inputs']},
						  'selection': lambda args: tuple(range(args['order'] if not isinstance(args.get('selection'),(int,np.integer)) else args.get('selection')+1)) if not isinstance(args.get('selection'),(np.ndarray)) else args['selection'] }
		args = {}
		args.update(texargs)
		...
		prefixes = {
			'constant': lambda string,latex,iloc,labels: labels['func']%(latex,'%s'%(','.join(['{%s}_{%s}'%(v,str(iloc) if iloc not in [None,'None',str(None)] else '') for v in args['groups']]))),
			'subscript': lambda string,latex,iloc,labels: r'{%s}_{%s}'%(latex,str(iloc)),
			'superscript': lambda string,latex,iloc,labels: r'{%s^{%s}'%(latex,str(iloc)),
			'monomial': lambda string,latex,iloc,labels: r'{%s}'%(latex),
			'polynomial': lambda string,latex,iloc,labels: r'{%s}'%(latex),
			'chebyshev': lambda string,latex,iloc,labels: r'{%s}'%(latex),
			'legendre': lambda string,latex,iloc,labels: r'{%s}'%(latex),
			'hermite': lambda string,latex,iloc,labels: r'{%s}'%(latex),
			'expansion': lambda string,latex,iloc,labels: r'{%s}'%(latex.replace('True',str(iloc) if iloc not in [None,'None',str(None)] else '')),
			'taylorseries': lambda string,latex,iloc,labels: r'{%s}'%(latex.replace('True',str(iloc) if iloc not in [None,'None',str(None)] else '')),
			'derivative': lambda string,latex,iloc,labels: r'{%s}'%(latex),
			'difference': lambda string,latex,iloc,labels: r'{%s}'%(latex),
			'deltavar': lambda string,latex,iloc,labels: r'%s{%s}'%(labels['delta'],latex),
			'Deltavar': lambda string,latex,iloc,labels: r'%s{%s}'%(labels['Delta'],latex),
			'coefficient': lambda string,latex,iloc,labels: labels['coefficient']%(labels['gamma'],latex),
			'factorial': lambda string,latex,iloc,labels: labels['factorial']%(str(iloc)),
			'iteration': lambda string,latex,iloc,labels: r'{%s}^{(%s)}'%(latex,iloc),
			}
		...
		def constants(variable,strings,args,labels,usetex):
			...
			values = variables
			# '%s%s'%(outputs[y],'(%s)'%(','.join(['{%s}_{%s}'%(v,str(i) if i is not None else '') for v in sorted(set([groups.get(v,inputs[v]) for v in inputs]),key=lambda i:list(inputs).index(i))])))
			return values
		...
		def derivative(symbol,strings,args,labels,usetex):
			...
			groups = {y: strings.get(y,args['groups'][y]) for y in args['groups']}
			groups = [v[1] for v in sorted(set([(v,groups.get(v,inputs[v])) for v in inputs]),key=lambda i:list(inputs).index(i[0]))]
		...
		def expansion(iloc,symbol,Symbol,strings,args,labels,usetex):
			...
			values = {
				...
				# '%s%s%s'%(
				# 	r'\frac{1}{%s}'%(labels['factorial']%(str(j))) if j>1 else '',
				# 	func%(symbol,str(j) if j>1 else '','%s%s'%(outputs[y],'(%s)'%(','.join(['{%s}_{%s}'%(v,str(iloc) if iloc is not None else '') for v in groups]))),
				# 		' '.join([r'%s %s'%(symbol,inputs[v]) for v in x]) if len(set([v for v in x]))>1 else r'{%s %s}^{%s}'%(symbol,inputs[[v for v in x][0]],str(j) if j>1 else '')),
				# 	' '.join([r'%s %s'%(Symbol,inputs[v]) for v in x]) if len(set([v for v in x]))>1 else r'{%s %s}^{%s}'%(Symbol,inputs[[v for v in x][0]],str(j) if j>1 else '')
				# )if usetex else (
				# 	'd^%s%s/%s %s'%(str(j) if j>1 else '',y,''.join(['d%s'%(v) for v in x]), ' '.join(['d%s'%(v) for v in x]))
				# )
				'%s'%(
					func%(symbol,str(j) if j>1 else '','%s%s'%(outputs[y],'(%s)'%(','.join(['{%s}_{%s}'%(v,str(iloc) if iloc is not None else '') for v in groups]))),
						' '.join([r'%s %s'%(symbol,inputs[v]) for v in x]) if len(set([v for v in x]))>1 else r'{%s %s}^{%s}'%(symbol,inputs[[v for v in x][0]],str(j) if j>1 else '')),
			...
			if args['bases'].get('taylorseries'):
				...
				values.update({
					...
					delimiter.join([w]*j)
						...
						r'\frac{%s}{%s}'%(labels['coefficient']%(labels['gamma'],''.join([r'%s'%(inputs[v]) for v in x]) if len(set([v for v in x]))>1 else r'{%s}^{%s}'%(inputs[[v for v in x][0]],str(j) if j>1 else '')
							),labels['factorial']%(j)) if j>1 else labels['coefficient']%(labels['gamma'],''.join([r'%s'%(inputs[v]) for v in x]) if len(set([v for v in x]))>1 else r'{%s}^{%s}'%(inputs[[v for v in x][0]],str(j) if j>1 else '')),
						func%(symbol,str(j) if j>1 else '','%s%s'%(outputs[y],'(%s)'%(','.join(['{%s}_{%s}'%(v,str(iloc) if iloc is not None else '') for v in groups]))),
							' '.join([r'%s %s'%(symbol,inputs[v]) for v in x]) if len(set([v for v in x]))>1 else r'{%s %s}^{%s}'%(symbol,inputs[[v for v in x][0]],str(j) if j>1 else '')),
						' '.join([r'%s %s'%(Symbol,inputs[v]) for v in x]) if len(set([v for v in x]))>1 else r'{%s %s}^{%s}'%(Symbol,inputs[[v for v in x][0]],str(j) if j>1 else '')
					) if usetex else (
