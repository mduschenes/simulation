...
sbatch -J job "--export=JOB_CWD=.,JOB_ENV=tensor,JOB_SRC=~/code/tensor/src,JOB_CMD=process.py,JOB_PROCESSES=1,JOB_ARGS=--data=**/data.hdf5 --plots=plot.json --processes=process.json --pwd=. --cwd=. --verbose,NUMPY_BACKEND=jax" < postprocess.slurm
